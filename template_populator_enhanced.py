...
class TemplatePopulator:
    ...
    def populate(self, data: Dict[str, Any], output_path: Path, 
        ...
        try:
            ...
            # Add bullet points to the materials section after saving
            if 'required_materials' in processed_data and processed_data['required_materials']:
                self._add_material_bullet_points(output_path, processed_data['required_materials'])
    ...
    def _add_material_bullet_points(self, doc_path: Path, materials: List[str]) -> bool:
        """
        Add properly formatted bullet points to the materials section of the populated document.
        
        Args:
            doc_path: Path to the document to add bullet points to
            materials: List of material items to add as bullet points
        ...
        try:
            ...
            self.logger.info(f"Adding material bullet points to document: {doc_path}")
            ...
            for i, para in enumerate(doc.paragraphs):
                if "MATERIALS REQUIRED" in para.text.upper():
                    ...
                    self.logger.info(f"Found materials section at paragraph {i}: {para.text}")
                    break
            
            if materials_section_index is None:
                self.logger.error("Could not find materials section in document")
                return False
            ...
            for i in range(materials_section_index + 1, min(materials_section_index + 20, len(doc.paragraphs))):
                ...
                if '•' in para.text or (para.style.name and 'List' in para.style.name):
                    self.logger.info(f"Found paragraph to remove at index {i}: {para.text}")
            ...
            for material in materials:
                ...
                if material_text.isupper() or "CURVE" in material_text.upper() or "ASSAY" in material_text.upper():
                    continue
                    
                # Truncate very long materials
                if len(material_text) > 150:
                    material_text = material_text[:150] + "..."
                    
                # Only add if not already in the list (case-insensitive)
                if not any(material_text.lower() in existing.lower() for existing in final_materials):
                    final_materials.append(material_text)
            ...
            # Save the document
            doc.save(doc_path)
            self.logger.info(f"Added {len(final_materials)} material bullet points to document")
            return True
            
        except Exception as e:
            self.logger.error(f"Error adding material bullet points: {e}")
            return False